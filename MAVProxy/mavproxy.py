...
from MAVProxy.modules.lib import mp_module, rline, textconsole
...
class MPStatus(object):
    ...
    def __init__(self):
        ...
        self.setup_mode = cmd_opts.setup
        self.mav_error = 0
        self.altitude = 0
...
def say_text(text, priority='important'):  # Priority doesn't seem used, should be keep ?
    """Text output - default function for say()."""
    mpstate.console.writeln(text)


def say(text, priority='important'):  # Priority doesn't seem used, should be keep ?
    ...
    def __init__(self):
        ...
        self.settings = MPSettings(
            [MPSetting('link', int, 1, 'Primary Link', tab='Link', range=(0, 4), increment=1),
             ...
             MPSetting('baudrate', int, cmd_opts.baudrate, 'baudrate for new links', range=(0, 10000000), increment=1),
             MPSetting('rtscts', bool, cmd_opts.rtscts, 'enable flow control'),
             MPSetting('select_timeout', float, 0.01, 'select timeout'),

             MPSetting('altreadout', int, 10, 'Altitude Readout', range=(0, 100), increment=1, tab='Announcements'),
             MPSetting('distreadout', int, 200, 'Distance Readout', range=(0, 10000), increment=1),

             MPSetting('moddebug', int, cmd_opts.moddebug, 'Module Debug Level', range=(0, 3), increment=1, tab='Debug'),
...
def process_stdin(line):
    ...
    cmd_args = shlex_quotes(line)
    cmd = cmd_args[0]
    while cmd in mpstate.aliases:
        line = mpstate.aliases[cmd]
        cmd_args = shlex.split(line) + cmd_args[1:]
        cmd = cmd_args[0]
    ...
    try:
        ...
        print("ERROR in command %s: %s" % (cmd_args[1:], str(e)))
        if mpstate.settings.moddebug > 1:
            traceback.print_exc()
...
def process_master(m):
    ...
    if m.first_byte and cmd_opts.auto_protocol:
...
def process_mavlink(slave):
    ...
    try:
        ...
        return
    try:
        if slave.first_byte and cmd_opts.auto_protocol:
...
def log_paths():
    ...
    if cmd_opts.aircraft is not None:
        ...
        if(cmd_opts.daemon):
            dirname = '/var/log/'
        if cmd_opts.mission is not None:
            print(cmd_opts.mission)
            dirname += "%s/logs/%s/Mission%s" % (cmd_opts.aircraft, time.strftime("%Y-%m-%d"), cmd_opts.mission)
        else:
            dirname += "%s/logs/%s" % (cmd_opts.aircraft, time.strftime("%Y-%m-%d"))
        ...
        if mpstate.continue_mode and highest is not None:
            fdir = highest
        elif os.path.exists(fdir):  # MIGHT BE NOT ASSIGNED
            print("Flight logs full")
            sys.exit(1)
        logname = 'flight.tlog'
        logdir = fdir  # MIGHT BE NOT ASSIGNED
    else:
        logname = os.path.basename(cmd_opts.logfile)
        dir_path = os.path.dirname(cmd_opts.logfile)
    ...
    while mpstate.status.exit is not True:
        try:
            if mpstate.status.exit is not True:
                ...
                mpstate.input_queue.put(line)  # Should be here ?
        except EOFError:
            mpstate.status.exit = True
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    mavutil.set_dialect(cmd_opts.dialect)
    ...
    mpstate.continue_mode = cmd_opts.continue_mode
    # queues for logging
    ...
    mpstate.settings.target_system = cmd_opts.TARGET_SYSTEM
    mpstate.settings.target_component = cmd_opts.TARGET_COMPONENT
    ...
    try:
        ...
        pass
    if cmd_opts.daemon:  # SIGINT breaks readline parsing - if we are interactive, just let things die
    ...
    mpstate.settings.source_system = cmd_opts.SOURCE_SYSTEM
    mpstate.settings.source_component = cmd_opts.SOURCE_COMPONENT

    # open master link
    for mdev in cmd_opts.master:
        if not mpstate.module('link').link_add(mdev):
            sys.exit(1)

    if not cmd_opts.master and len(serial_list) == 1:  # Do we really want to auto-connect to the only serial port available?
        print("Connecting to %s" % serial_list[0])
        mpstate.module('link').link_add(serial_list[0].device)
    elif not cmd_opts.master:
        wifi_device = '0.0.0.0:14550'
        mpstate.module('link').link_add(wifi_device)

    # open any mavlink output ports
    for port in cmd_opts.output:
        mpstate.mav_outputs.append(mavutil.mavlink_connection(port, baud=int(cmd_opts.baudrate), input=False))

    if cmd_opts.sitl:
        mpstate.sitl_output = mavutil.mavudp(cmd_opts.sitl, input=False)

    mpstate.settings.streamrate = cmd_opts.streamrate
    mpstate.settings.streamrate2 = cmd_opts.streamrate

    if cmd_opts.state_basedir is not None:
        mpstate.settings.state_basedir = cmd_opts.state_basedir
    ...
    if not cmd_opts.setup:
        # some core functionality is in modules
        standard_modules = cmd_opts.default_modules.split(',')
    ...
    for module in cmd_opts.load_module:
        modlist = module.split(',')
        for modules in modlist:
            process_stdin('module load %s' % modules)

    if 'HOME' in os.environ and not cmd_opts.setup:
        start_script = os.path.join(os.environ['HOME'], ".mavinit.scr")
        if os.path.exists(start_script):
            run_script(start_script)
    if 'LOCALAPPDATA' in os.environ and not cmd_opts.setup:
        start_script = os.path.join(os.environ['LOCALAPPDATA'], "MAVProxy", "mavinit.scr")
        if os.path.exists(start_script):
            run_script(start_script)

    if cmd_opts.aircraft is not None:
        start_script = os.path.join(cmd_opts.aircraft, "mavinit.scr")
        if os.path.exists(start_script):
            run_script(start_script)
        else:
            print("no script %s" % start_script)

    if cmd_opts.cmd is not None:
        for cstr in cmd_opts.cmd:
            cmds = cstr.split(';')
            for c in cmds:
                process_stdin(c)
